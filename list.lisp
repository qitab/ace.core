...
The old LIST is not modified in this process but may share the
tail with the new resulting LIST. If no item is removed from
the old LIST, it will be returned as is. Any following destructive
operation on the new LIST may affect a part of the old one as well.
...
The REST parameters are:
 ...
 COUNT - if provided, the maximum count of elements to be deleted,
...
(defmacro removef-if-not (predicate list &rest rest
                          &key from-end start end count key
                          &environment env)
  "Remove all elements from the LIST that do NOT match the PREDICATE.
LIST needs to be a place and is assigned the new returned LIST.
The old LIST is not modified in this process but may share the
tail with the new resulting LIST. If no item is removed from
the old LIST, it will be returned as is. Any following destructive
...
(defmacro deletef-if-not (predicate list &rest rest
                          &key from-end start end count key
                          &environment env)
  "Destructively delete all elements from the LIST that do NOT match the PREDICATE.
LIST needs to be a place and is assigned the returned modified LIST.
